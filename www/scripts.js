  ...
  ons.notification.prompt("Insert new To-Do Item label.", {
    ...
    callback: function(label){
      ...
      if(todoStorage.add(label)){
        ...
        ons.notification.alert("Failed to add the item to the list.")
      }
    }.bind(this)
  });
};
...
todo.refresh = function(){
  ...
  this.list.innerHTML = items.map(function(item){
    return document.querySelector("ons-list-item").innerHTML.replace("{{label}}", item.label).replace("{{checked}}", item.status === "completed" ? "checked" : "");
  }).join("");
...
todo.removeItemPrompt = function(label){
    ons.notification.confirm("Are you sure you'd like to remove " + label + " from the list?", {
    title: "Remove Item?",
    callback: function(answer){
      if(answer === 1) {
       if(todoStorage.remove(label)){
         ...
         ons.notification.alert("Failed to remove item from list")
       }
     }
    }.bind(this)
  });
